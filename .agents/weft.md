...
The real Weft CLI is **not** the `weft` that may be on `$PATH` (that's an unrelated tool). Use the maestro binary:

```bash
WEFT=~/Programming/maestro/weft/bin/weft
$WEFT version
...
> **Which project id?** There are multiple projects on the server. Harbor's is `proj_2fb1e550…`. Verify by grouping features by `project_id` (`$WEFT features list --json`) and confirming known Harbor features (e.g. `user-audit-trail`, `kms-provider-integration`) live under it. Do **not** confuse this with `.hippo/project.yaml`'s `proj_c52ddf65` — that is Hippo's project, unrelated to Weft.
...
```bash
WEFT=~/Programming/maestro/weft/bin/weft
HARBOR_PROJ=proj_2fb1e550-568b-4102-9285-ecbc17bb7997
...
- [ ] Using the maestro binary (`~/Programming/maestro/weft/bin/weft`), not a `$PATH` `weft`?
- [ ] Launching under Harbor's project `proj_2fb1e550…` (verified, not `.hippo`'s id)?
