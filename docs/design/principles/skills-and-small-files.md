...
**This is enforced automatically.** `tools/lint/filesize` (wired into `make agent-check`) flags any non-generated Go file over 400 lines (500 for `_test.go`) or any `docs/design/**/*.md` file over 2,000 words — the same silent-failure shape §1.11 closed for build-tag/lint coupling, applied here to file size. It's a floor, not a substitute for judgment: the `@harbor-reviewer` agent still flags files that mix multiple concerns (even under the threshold) and suggests splitting them along a package boundary.
