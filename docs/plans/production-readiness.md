...
| ~~**1.5**~~ | ~~**Unauthenticated admin endpoints**~~ — ~~`POST /admin/revoke-jwt` and `/admin/keys/*` have **zero `Authorization` header checks**, and they sit on the internet-facing `harbor-hot` binary. Anyone who can reach the pod can revoke tokens or manipulate keys.~~ | ~~`internal/oidcapi/revoke_jwt.go`; `internal/oidcapi/admin_keys.go`~~ | ✅ **RESOLVED** by `admin-endpoint-auth` (`feat_96d1d95a`). `AdminAuthMiddleware` + `WithAdminAuth` dispatcher enforce `Authorization: Bearer <token>` (constant-time, SHA-256 hashed) on both admin paths; ADMIN_API_TOKEN boot guard fails closed when unset; `/admin/` blocked at ingress; tight rate limits added. |
...
| 3 | ~~`admin-endpoint-auth`~~ | ~~**P0**~~ | ~~1.5~~ | 2–4 h | ✅ **DONE** (`feat_96d1d95a`) |
