...
The real resolver flow: WebAuthn login ceremony ⇒ authenticated `user_id` ⇒ crypto (unwrap DEK, decrypt `pairwise_secret`) ⇒ `ClientRegistry.sector_id` ⇒ `identity.DerivePPID(pairwise_secret, sector_id, user_id)` ⇒ `sub` ⇒ `GrantStore` (find-or-create consent grant).
...
**Then** it returns the derived pairwise `sub` and `approved = true` with a find-or-create consent grant
